...
clc; clear; close all;
%% Problem Definition
ObjectiveFunction=@(x) Sphere(x); funcname='Sphere'; % Objective Function
...
for it=1:MaxIt
    for i=1:nPop
        ...
        rgbest=norm(GlobalBest.Position-Mayfly(i).Position);
        e=unifrnd(-1,+1,ProblemSize);
        % Update Velocity
    ...
    for k=1:nc/2
        ...
        MayflyOffspring(k,1).Best.Cost = MayflyOffspring(k,1).Position;
        MayflyOffspring(k,1).Velocity= zeros(ProblemSize);
        MayflyOffspring(k,2).Best.Position = MayflyOffspring(k,2).Position;
        MayflyOffspring(k,2).Best.Cost = MayflyOffspring(k,2).Position;
    ...
    for k=1:nm
        ...
        MutMayflies(k).Best.Cost = MutMayflies(k).Position;
    ...
    disp([methname ': Iteration ' num2str(it) '  Evaluations ' num2str(funccount)  ': Best Cost = ' num2str(BestSolution(it))]);
    g=g*gdamp;
